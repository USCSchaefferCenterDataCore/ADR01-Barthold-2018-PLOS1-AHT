...
•	These classes exist, but there are no claims: loopso a2rbbblo loopsothia cchbthia
*/

////////////////////////////////////////////////////////////////////////////////
/////////			NDC level file with loopso	////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
gen loopso = 1 if strpos(hic3_desc, "LOOP DIURETIC")
...
///////////////// Array
forvalues d = 1/365{
	gen loopso_a`d' = 1 if `d'>=doy_srvc & `d'<=(doy_srvc+dayssply2) & loopso==1
}

drop doy_srvc
xfill loopso_a*, i(idn)

/////////////////bene-class timing
egen loopso_fdays_2006 = sum(dayssply), by(idn)
egen loopso_clms_2006 = count(dayssply), by(idn)
egen loopso_minfilldt_2006 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2006 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2006 = loopso_maxfilldt_2006 - loopso_minfilldt_2006 + 1
...
forvalues d = 1/365{
	replace snf_push = snf_push + 1 if loopso_a`d'==1 & snf_a`d'==1

	//if the loopso_a`d' is 1, do nothing (already added it to the snf_push)
	//if that loopso_a spot is empty:
	gen v1 = 1 if snf_push>0 & loopso_a`d'==.
	replace loopso_a`d' = 1 if v1==1
...
forvalues d = 1/365{
	replace ips_push = ips_push + 1 if loopso_a`d'==1 & ips_a`d'==1

	//if the loopso_a`d' is 1, do nothing (already added it to the ips_push)
	//if that loopso_a spot is not full:
	gen v1 = 1 if ips_push>0 & loopso_a`d'==.
	replace loopso_a`d' = 1 if v1==1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2006 = loopso_pdays_2006 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2006 = 365 if loopso_pdays_2006>365
...
egen loopso_push_2006 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2006 = 0 if loopso_push_2006==.
replace loopso_push_2006 = 0 if loopso_push_2006<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2006p.dta", replace

keep bene_id loopso_push_2006
tempfile push
save `push', replace
...
replace pushstock = pushstock + loopso_push_2006 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2007 = sum(dayssply), by(idn)
egen loopso_clms_2007 = count(dayssply), by(idn)
egen loopso_minfilldt_2007 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2007 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2007 = loopso_maxfilldt_2007 - loopso_minfilldt_2007 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2007 = loopso_pdays_2007 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2007 = 365 if loopso_pdays_2007>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2007 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2007 = 0 if loopso_push_2007==.
replace loopso_push_2007 = 0 if loopso_push_2007<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2007p.dta", replace

keep bene_id loopso_push_2007
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + loopso_push_2007 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2008 = sum(dayssply), by(idn)
egen loopso_clms_2008 = count(dayssply), by(idn)
egen loopso_minfilldt_2008 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2008 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2008 = loopso_maxfilldt_2008 - loopso_minfilldt_2008 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2008 = loopso_pdays_2008 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2008 = 365 if loopso_pdays_2008>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2008 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2008 = 0 if loopso_push_2008==.
replace loopso_push_2008 = 0 if loopso_push_2008<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2008p.dta", replace

keep bene_id loopso_push_2008
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + loopso_push_2008 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2009 = sum(dayssply), by(idn)
egen loopso_clms_2009 = count(dayssply), by(idn)
egen loopso_minfilldt_2009 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2009 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2009 = loopso_maxfilldt_2009 - loopso_minfilldt_2009 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2009 = loopso_pdays_2009 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2009 = 365 if loopso_pdays_2009>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2009 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2009 = 0 if loopso_push_2009==.
replace loopso_push_2009 = 0 if loopso_push_2009<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2009p.dta", replace

keep bene_id loopso_push_2009
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + loopso_push_2009 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2010 = sum(dayssply), by(idn)
egen loopso_clms_2010 = count(dayssply), by(idn)
egen loopso_minfilldt_2010 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2010 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2010 = loopso_maxfilldt_2010 - loopso_minfilldt_2010 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2010 = loopso_pdays_2010 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2010 = 365 if loopso_pdays_2010>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2010 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2010 = 0 if loopso_push_2010==.
replace loopso_push_2010 = 0 if loopso_push_2010<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2010p.dta", replace

keep bene_id loopso_push_2010
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + loopso_push_2010 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2011 = sum(dayssply), by(idn)
egen loopso_clms_2011 = count(dayssply), by(idn)
egen loopso_minfilldt_2011 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2011 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2011 = loopso_maxfilldt_2011 - loopso_minfilldt_2011 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2011 = loopso_pdays_2011 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2011 = 365 if loopso_pdays_2011>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2011 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2011 = 0 if loopso_push_2011==.
replace loopso_push_2011 = 0 if loopso_push_2011<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2011p.dta", replace

keep bene_id loopso_push_2011
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + loopso_push_2011 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2012 = sum(dayssply), by(idn)
egen loopso_clms_2012 = count(dayssply), by(idn)
egen loopso_minfilldt_2012 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2012 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2012 = loopso_maxfilldt_2012 - loopso_minfilldt_2012 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2012 = loopso_pdays_2012 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2012 = 365 if loopso_pdays_2012>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2012 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2012 = 0 if loopso_push_2012==.
replace loopso_push_2012 = 0 if loopso_push_2012<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2012p.dta", replace

keep bene_id loopso_push_2012
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + loopso_push_2012 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen loopso_fdays_2013 = sum(dayssply), by(idn)
egen loopso_clms_2013 = count(dayssply), by(idn)
egen loopso_minfilldt_2013 = min(srvc_dt), by(idn)
egen loopso_maxfilldt_2013 = max(srvc_dt), by(idn)
gen loopso_fillperiod_2013 = loopso_maxfilldt_2013 - loopso_minfilldt_2013 + 1
...
*/
forvalues d = 1/365{
	replace loopso_pdays_2013 = loopso_pdays_2013 + 1 if loopso_a`d'==1
}
drop loopso_a*
replace loopso_pdays_2013 = 365 if loopso_pdays_2013>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen loopso_push_2013 = rowmax(lastfill_push inyear_push) 
replace loopso_push_2013 = 0 if loopso_push_2013==.
replace loopso_push_2013 = 0 if loopso_push_2013<0

keep bene_id loopso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_2013p.dta", replace
...
gen yloopso2008 = 1
merge 1:1 bene_id using `loopso'
drop _m
save `loopso', replace
...
foreach i in `years'{
	replace loopso_firstyoo = `i' if yloopso`i'==1
}
local years "2007 2008 2009 2010 2011 2012 2013"
foreach i in `years'{
	replace loopso_lastyoo = `i' if yloopso`i'==1
}
gen loopso_yearcount = loopso_lastyoo - loopso_firstyoo + 1

//utilization variables 
forvalues i = 2007/2013{
	replace loopso_fillperiod_`i' = 0 if loopso_fillperiod_`i'==.
	replace loopso_clms_`i' = 0 if loopso_clms_`i'==.
	replace loopso_fdays_`i' = 0 if loopso_fdays_`i'==.
	replace loopso_pdays_`i' = 0 if loopso_pdays_`i'==.
}	
//minfilldt maxfilldt push

//total utilization
gen loopso_clms = loopso_clms_2007+loopso_clms_2008+loopso_clms_2009+loopso_clms_2010+loopso_clms_2011+loopso_clms_2012+loopso_clms_2013
gen loopso_fdays = loopso_fdays_2007+loopso_fdays_2008+loopso_fdays_2009+loopso_fdays_2010+loopso_fdays_2011+loopso_fdays_2012+loopso_fdays_2013
gen loopso_pdays = loopso_pdays_2007+loopso_pdays_2008+loopso_pdays_2009+loopso_pdays_2010+loopso_pdays_2011+loopso_pdays_2012+loopso_pdays_2013

//timing variables 
egen loopso_minfilldt = rowmin(loopso_minfilldt_2007 loopso_minfilldt_2008 loopso_minfilldt_2009 loopso_minfilldt_2010 loopso_minfilldt_2011 loopso_minfilldt_2012 loopso_minfilldt_2013)
egen loopso_maxfilldt = rowmax(loopso_maxfilldt_2007 loopso_maxfilldt_2008 loopso_maxfilldt_2009 loopso_maxfilldt_2010 loopso_maxfilldt_2011 loopso_maxfilldt_2012 loopso_maxfilldt_2013)

gen loopso_fillperiod = loopso_maxfilldt - loopso_minfilldt + 1

gen loopso_pdayspy = loopso_pdays/loopso_yearcount
gen loopso_fdayspy = loopso_fdays/loopso_yearcount
gen loopso_clmspy = loopso_clms/loopso_yearcount

sort bene_id
drop if bene_id==bene_id[_n-1]

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/loopso_0713p.dta", replace
