...
•	These classes exist, but there are no claims: aceicchb a2rbbblo aceicchbthia cchbthia
*/
...
gen aceicchb = 1 if hic3_desc=="ACE INHIBITOR-CALCIUM CHANNEL BLOCKER COMBINATION" 

keep if aceicchb==1 

tempfile aceicchb
save `aceicchb', replace
...
///////////////// Array
forvalues d = 1/365{
	gen aceicchb_a`d' = 1 if `d'>=doy_srvc & `d'<=(doy_srvc+dayssply2) & aceicchb==1
}

drop doy_srvc
xfill aceicchb_a*, i(idn)

/////////////////bene-class timing
egen aceicchb_fdays_2006 = sum(dayssply), by(idn)
egen aceicchb_clms_2006 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2006 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2006 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2006 = aceicchb_maxfilldt_2006 - aceicchb_minfilldt_2006 + 1
...
forvalues d = 1/365{
	replace snf_push = snf_push + 1 if aceicchb_a`d'==1 & snf_a`d'==1

	//if the aceicchb_a`d' is 1, do nothing (already added it to the snf_push)
	//if that aceicchb_a spot is empty:
	gen v1 = 1 if snf_push>0 & aceicchb_a`d'==.
	replace aceicchb_a`d' = 1 if v1==1
...
forvalues d = 1/365{
	replace ips_push = ips_push + 1 if aceicchb_a`d'==1 & ips_a`d'==1

	//if the aceicchb_a`d' is 1, do nothing (already added it to the ips_push)
	//if that aceicchb_a spot is not full:
	gen v1 = 1 if ips_push>0 & aceicchb_a`d'==.
	replace aceicchb_a`d' = 1 if v1==1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2006 = aceicchb_pdays_2006 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2006 = 365 if aceicchb_pdays_2006>365
...
egen aceicchb_push_2006 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2006 = 0 if aceicchb_push_2006==.
replace aceicchb_push_2006 = 0 if aceicchb_push_2006<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2006p.dta", replace

keep bene_id aceicchb_push_2006
tempfile push
save `push', replace
...
replace pushstock = pushstock + aceicchb_push_2006 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2007 = sum(dayssply), by(idn)
egen aceicchb_clms_2007 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2007 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2007 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2007 = aceicchb_maxfilldt_2007 - aceicchb_minfilldt_2007 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2007 = aceicchb_pdays_2007 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2007 = 365 if aceicchb_pdays_2007>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2007 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2007 = 0 if aceicchb_push_2007==.
replace aceicchb_push_2007 = 0 if aceicchb_push_2007<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2007p.dta", replace

keep bene_id aceicchb_push_2007
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceicchb_push_2007 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2008 = sum(dayssply), by(idn)
egen aceicchb_clms_2008 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2008 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2008 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2008 = aceicchb_maxfilldt_2008 - aceicchb_minfilldt_2008 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2008 = aceicchb_pdays_2008 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2008 = 365 if aceicchb_pdays_2008>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2008 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2008 = 0 if aceicchb_push_2008==.
replace aceicchb_push_2008 = 0 if aceicchb_push_2008<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2008p.dta", replace

keep bene_id aceicchb_push_2008
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceicchb_push_2008 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2009 = sum(dayssply), by(idn)
egen aceicchb_clms_2009 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2009 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2009 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2009 = aceicchb_maxfilldt_2009 - aceicchb_minfilldt_2009 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2009 = aceicchb_pdays_2009 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2009 = 365 if aceicchb_pdays_2009>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2009 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2009 = 0 if aceicchb_push_2009==.
replace aceicchb_push_2009 = 0 if aceicchb_push_2009<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2009p.dta", replace

keep bene_id aceicchb_push_2009
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceicchb_push_2009 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2010 = sum(dayssply), by(idn)
egen aceicchb_clms_2010 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2010 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2010 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2010 = aceicchb_maxfilldt_2010 - aceicchb_minfilldt_2010 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2010 = aceicchb_pdays_2010 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2010 = 365 if aceicchb_pdays_2010>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2010 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2010 = 0 if aceicchb_push_2010==.
replace aceicchb_push_2010 = 0 if aceicchb_push_2010<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2010p.dta", replace

keep bene_id aceicchb_push_2010
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceicchb_push_2010 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2011 = sum(dayssply), by(idn)
egen aceicchb_clms_2011 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2011 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2011 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2011 = aceicchb_maxfilldt_2011 - aceicchb_minfilldt_2011 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2011 = aceicchb_pdays_2011 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2011 = 365 if aceicchb_pdays_2011>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2011 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2011 = 0 if aceicchb_push_2011==.
replace aceicchb_push_2011 = 0 if aceicchb_push_2011<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2011p.dta", replace

keep bene_id aceicchb_push_2011
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceicchb_push_2011 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2012 = sum(dayssply), by(idn)
egen aceicchb_clms_2012 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2012 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2012 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2012 = aceicchb_maxfilldt_2012 - aceicchb_minfilldt_2012 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2012 = aceicchb_pdays_2012 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2012 = 365 if aceicchb_pdays_2012>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2012 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2012 = 0 if aceicchb_push_2012==.
replace aceicchb_push_2012 = 0 if aceicchb_push_2012<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2012p.dta", replace

keep bene_id aceicchb_push_2012
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceicchb_push_2012 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceicchb_fdays_2013 = sum(dayssply), by(idn)
egen aceicchb_clms_2013 = count(dayssply), by(idn)
egen aceicchb_minfilldt_2013 = min(srvc_dt), by(idn)
egen aceicchb_maxfilldt_2013 = max(srvc_dt), by(idn)
gen aceicchb_fillperiod_2013 = aceicchb_maxfilldt_2013 - aceicchb_minfilldt_2013 + 1
...
*/
forvalues d = 1/365{
	replace aceicchb_pdays_2013 = aceicchb_pdays_2013 + 1 if aceicchb_a`d'==1
}
drop aceicchb_a*
replace aceicchb_pdays_2013 = 365 if aceicchb_pdays_2013>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceicchb_push_2013 = rowmax(lastfill_push inyear_push) 
replace aceicchb_push_2013 = 0 if aceicchb_push_2013==.
replace aceicchb_push_2013 = 0 if aceicchb_push_2013<0

keep bene_id aceicchb* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_2013p.dta", replace

////////////////////////////////////////////////////////////////////////////////
/////////////// 	Merge aceicchb 07-13	////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
gen yaceicchb2008 = 1
merge 1:1 bene_id using `aceicchb'
drop _m
save `aceicchb', replace
...
//timing variables
gen aceicchb_firstyoo=.
gen aceicchb_lastyoo=.
local years "2013 2012 2011 2010 2009 2008 2007"
foreach i in `years'{
	replace aceicchb_firstyoo = `i' if yaceicchb`i'==1
}
local years "2007 2008 2009 2010 2011 2012 2013"
foreach i in `years'{
	replace aceicchb_lastyoo = `i' if yaceicchb`i'==1
}
gen aceicchb_yearcount = aceicchb_lastyoo - aceicchb_firstyoo + 1

//utilization variables 
forvalues i = 2007/2013{
	replace aceicchb_fillperiod_`i' = 0 if aceicchb_fillperiod_`i'==.
	replace aceicchb_clms_`i' = 0 if aceicchb_clms_`i'==.
	replace aceicchb_fdays_`i' = 0 if aceicchb_fdays_`i'==.
	replace aceicchb_pdays_`i' = 0 if aceicchb_pdays_`i'==.
}	
//minfilldt maxfilldt push

//total utilization
gen aceicchb_clms = aceicchb_clms_2007+aceicchb_clms_2008+aceicchb_clms_2009+aceicchb_clms_2010+aceicchb_clms_2011+aceicchb_clms_2012+aceicchb_clms_2013
gen aceicchb_fdays = aceicchb_fdays_2007+aceicchb_fdays_2008+aceicchb_fdays_2009+aceicchb_fdays_2010+aceicchb_fdays_2011+aceicchb_fdays_2012+aceicchb_fdays_2013
gen aceicchb_pdays = aceicchb_pdays_2007+aceicchb_pdays_2008+aceicchb_pdays_2009+aceicchb_pdays_2010+aceicchb_pdays_2011+aceicchb_pdays_2012+aceicchb_pdays_2013

//timing variables 
egen aceicchb_minfilldt = rowmin(aceicchb_minfilldt_2007 aceicchb_minfilldt_2008 aceicchb_minfilldt_2009 aceicchb_minfilldt_2010 aceicchb_minfilldt_2011 aceicchb_minfilldt_2012 aceicchb_minfilldt_2013)
egen aceicchb_maxfilldt = rowmax(aceicchb_maxfilldt_2007 aceicchb_maxfilldt_2008 aceicchb_maxfilldt_2009 aceicchb_maxfilldt_2010 aceicchb_maxfilldt_2011 aceicchb_maxfilldt_2012 aceicchb_maxfilldt_2013)

gen aceicchb_fillperiod = aceicchb_maxfilldt - aceicchb_minfilldt + 1

gen aceicchb_pdayspy = aceicchb_pdays/aceicchb_yearcount
gen aceicchb_fdayspy = aceicchb_fdays/aceicchb_yearcount
gen aceicchb_clmspy = aceicchb_clms/aceicchb_yearcount

sort bene_id
drop if bene_id==bene_id[_n-1]

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceicchb_0713p.dta", replace
