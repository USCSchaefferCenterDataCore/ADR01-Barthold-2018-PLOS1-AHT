...
•	These classes exist, but there are no claims: thiaso a2rbbblo thiasothia cchbthia
*/
...
gen thiaso = 1 if hic3_desc=="THIAZIDE AND RELATED DIURETICS" | ///
	hic3_desc=="RENIN INHIBITOR,DIRECT AND THIAZIDE DIURETIC COMB" | ///
	hic3_desc=="ALPHA-ADRENERGIC BLOCKING AGENT/THIAZIDE COMB"
...
///////////////// Array
forvalues d = 1/365{
	gen thiaso_a`d' = 1 if `d'>=doy_srvc & `d'<=(doy_srvc+dayssply2) & thiaso==1
}

drop doy_srvc
xfill thiaso_a*, i(idn)

/////////////////bene-class timing
egen thiaso_fdays_2006 = sum(dayssply), by(idn)
egen thiaso_clms_2006 = count(dayssply), by(idn)
egen thiaso_minfilldt_2006 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2006 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2006 = thiaso_maxfilldt_2006 - thiaso_minfilldt_2006 + 1
...
forvalues d = 1/365{
	replace snf_push = snf_push + 1 if thiaso_a`d'==1 & snf_a`d'==1

	//if the thiaso_a`d' is 1, do nothing (already added it to the snf_push)
	//if that thiaso_a spot is empty:
	gen v1 = 1 if snf_push>0 & thiaso_a`d'==.
	replace thiaso_a`d' = 1 if v1==1
...
forvalues d = 1/365{
	replace ips_push = ips_push + 1 if thiaso_a`d'==1 & ips_a`d'==1

	//if the thiaso_a`d' is 1, do nothing (already added it to the ips_push)
	//if that thiaso_a spot is not full:
	gen v1 = 1 if ips_push>0 & thiaso_a`d'==.
	replace thiaso_a`d' = 1 if v1==1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2006 = thiaso_pdays_2006 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2006 = 365 if thiaso_pdays_2006>365
...
egen thiaso_push_2006 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2006 = 0 if thiaso_push_2006==.
replace thiaso_push_2006 = 0 if thiaso_push_2006<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2006p.dta", replace

keep bene_id thiaso_push_2006
tempfile push
save `push', replace
...
replace pushstock = pushstock + thiaso_push_2006 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2007 = sum(dayssply), by(idn)
egen thiaso_clms_2007 = count(dayssply), by(idn)
egen thiaso_minfilldt_2007 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2007 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2007 = thiaso_maxfilldt_2007 - thiaso_minfilldt_2007 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2007 = thiaso_pdays_2007 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2007 = 365 if thiaso_pdays_2007>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2007 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2007 = 0 if thiaso_push_2007==.
replace thiaso_push_2007 = 0 if thiaso_push_2007<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2007p.dta", replace

keep bene_id thiaso_push_2007
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + thiaso_push_2007 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2008 = sum(dayssply), by(idn)
egen thiaso_clms_2008 = count(dayssply), by(idn)
egen thiaso_minfilldt_2008 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2008 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2008 = thiaso_maxfilldt_2008 - thiaso_minfilldt_2008 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2008 = thiaso_pdays_2008 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2008 = 365 if thiaso_pdays_2008>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2008 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2008 = 0 if thiaso_push_2008==.
replace thiaso_push_2008 = 0 if thiaso_push_2008<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2008p.dta", replace

keep bene_id thiaso_push_2008
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + thiaso_push_2008 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2009 = sum(dayssply), by(idn)
egen thiaso_clms_2009 = count(dayssply), by(idn)
egen thiaso_minfilldt_2009 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2009 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2009 = thiaso_maxfilldt_2009 - thiaso_minfilldt_2009 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2009 = thiaso_pdays_2009 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2009 = 365 if thiaso_pdays_2009>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2009 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2009 = 0 if thiaso_push_2009==.
replace thiaso_push_2009 = 0 if thiaso_push_2009<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2009p.dta", replace

keep bene_id thiaso_push_2009
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + thiaso_push_2009 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2010 = sum(dayssply), by(idn)
egen thiaso_clms_2010 = count(dayssply), by(idn)
egen thiaso_minfilldt_2010 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2010 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2010 = thiaso_maxfilldt_2010 - thiaso_minfilldt_2010 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2010 = thiaso_pdays_2010 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2010 = 365 if thiaso_pdays_2010>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2010 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2010 = 0 if thiaso_push_2010==.
replace thiaso_push_2010 = 0 if thiaso_push_2010<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2010p.dta", replace

keep bene_id thiaso_push_2010
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + thiaso_push_2010 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2011 = sum(dayssply), by(idn)
egen thiaso_clms_2011 = count(dayssply), by(idn)
egen thiaso_minfilldt_2011 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2011 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2011 = thiaso_maxfilldt_2011 - thiaso_minfilldt_2011 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2011 = thiaso_pdays_2011 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2011 = 365 if thiaso_pdays_2011>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2011 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2011 = 0 if thiaso_push_2011==.
replace thiaso_push_2011 = 0 if thiaso_push_2011<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2011p.dta", replace

keep bene_id thiaso_push_2011
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + thiaso_push_2011 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2012 = sum(dayssply), by(idn)
egen thiaso_clms_2012 = count(dayssply), by(idn)
egen thiaso_minfilldt_2012 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2012 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2012 = thiaso_maxfilldt_2012 - thiaso_minfilldt_2012 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2012 = thiaso_pdays_2012 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2012 = 365 if thiaso_pdays_2012>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2012 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2012 = 0 if thiaso_push_2012==.
replace thiaso_push_2012 = 0 if thiaso_push_2012<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2012p.dta", replace

keep bene_id thiaso_push_2012
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + thiaso_push_2012 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen thiaso_fdays_2013 = sum(dayssply), by(idn)
egen thiaso_clms_2013 = count(dayssply), by(idn)
egen thiaso_minfilldt_2013 = min(srvc_dt), by(idn)
egen thiaso_maxfilldt_2013 = max(srvc_dt), by(idn)
gen thiaso_fillperiod_2013 = thiaso_maxfilldt_2013 - thiaso_minfilldt_2013 + 1
...
*/
forvalues d = 1/365{
	replace thiaso_pdays_2013 = thiaso_pdays_2013 + 1 if thiaso_a`d'==1
}
drop thiaso_a*
replace thiaso_pdays_2013 = 365 if thiaso_pdays_2013>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen thiaso_push_2013 = rowmax(lastfill_push inyear_push) 
replace thiaso_push_2013 = 0 if thiaso_push_2013==.
replace thiaso_push_2013 = 0 if thiaso_push_2013<0

keep bene_id thiaso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_2013p.dta", replace
...
gen ythiaso2008 = 1
merge 1:1 bene_id using `thiaso'
drop _m
save `thiaso', replace
...
foreach i in `years'{
	replace thiaso_firstyoo = `i' if ythiaso`i'==1
}
local years "2007 2008 2009 2010 2011 2012 2013"
foreach i in `years'{
	replace thiaso_lastyoo = `i' if ythiaso`i'==1
}
gen thiaso_yearcount = thiaso_lastyoo - thiaso_firstyoo + 1

//utilization variables 
forvalues i = 2007/2013{
	replace thiaso_fillperiod_`i' = 0 if thiaso_fillperiod_`i'==.
	replace thiaso_clms_`i' = 0 if thiaso_clms_`i'==.
	replace thiaso_fdays_`i' = 0 if thiaso_fdays_`i'==.
	replace thiaso_pdays_`i' = 0 if thiaso_pdays_`i'==.
}	
//minfilldt maxfilldt push

//total utilization
gen thiaso_clms = thiaso_clms_2007+thiaso_clms_2008+thiaso_clms_2009+thiaso_clms_2010+thiaso_clms_2011+thiaso_clms_2012+thiaso_clms_2013
gen thiaso_fdays = thiaso_fdays_2007+thiaso_fdays_2008+thiaso_fdays_2009+thiaso_fdays_2010+thiaso_fdays_2011+thiaso_fdays_2012+thiaso_fdays_2013
gen thiaso_pdays = thiaso_pdays_2007+thiaso_pdays_2008+thiaso_pdays_2009+thiaso_pdays_2010+thiaso_pdays_2011+thiaso_pdays_2012+thiaso_pdays_2013

//timing variables 
egen thiaso_minfilldt = rowmin(thiaso_minfilldt_2007 thiaso_minfilldt_2008 thiaso_minfilldt_2009 thiaso_minfilldt_2010 thiaso_minfilldt_2011 thiaso_minfilldt_2012 thiaso_minfilldt_2013)
egen thiaso_maxfilldt = rowmax(thiaso_maxfilldt_2007 thiaso_maxfilldt_2008 thiaso_maxfilldt_2009 thiaso_maxfilldt_2010 thiaso_maxfilldt_2011 thiaso_maxfilldt_2012 thiaso_maxfilldt_2013)

gen thiaso_fillperiod = thiaso_maxfilldt - thiaso_minfilldt + 1

gen thiaso_pdayspy = thiaso_pdays/thiaso_yearcount
gen thiaso_fdayspy = thiaso_fdays/thiaso_yearcount
gen thiaso_clmspy = thiaso_clms/thiaso_yearcount

sort bene_id
drop if bene_id==bene_id[_n-1]

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/thiaso_0713p.dta", replace
