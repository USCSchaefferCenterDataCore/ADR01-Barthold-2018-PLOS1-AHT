...
•	These classes exist, but there are no claims: aceiso a2rbbblo aceisothia cchbthia
*/
...
gen aceiso = 1 if hic3_desc=="ANTIHYPERTENSIVES, ACE INHIBITORS" | ///
	hic3_desc=="ANTIHYPERTENSIVES,ACE INHIBITOR/DIETARY SUPP.COMB."
...
///////////////// Array
forvalues d = 1/365{
	gen aceiso_a`d' = 1 if `d'>=doy_srvc & `d'<=(doy_srvc+dayssply2) & aceiso==1
}

drop doy_srvc
xfill aceiso_a*, i(idn)

/////////////////bene-class timing
egen aceiso_fdays_2006 = sum(dayssply), by(idn)
egen aceiso_clms_2006 = count(dayssply), by(idn)
egen aceiso_minfilldt_2006 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2006 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2006 = aceiso_maxfilldt_2006 - aceiso_minfilldt_2006 + 1
...
forvalues d = 1/365{
	replace snf_push = snf_push + 1 if aceiso_a`d'==1 & snf_a`d'==1

	//if the aceiso_a`d' is 1, do nothing (already added it to the snf_push)
	//if that aceiso_a spot is empty:
	gen v1 = 1 if snf_push>0 & aceiso_a`d'==.
	replace aceiso_a`d' = 1 if v1==1
...
forvalues d = 1/365{
	replace ips_push = ips_push + 1 if aceiso_a`d'==1 & ips_a`d'==1

	//if the aceiso_a`d' is 1, do nothing (already added it to the ips_push)
	//if that aceiso_a spot is not full:
	gen v1 = 1 if ips_push>0 & aceiso_a`d'==.
	replace aceiso_a`d' = 1 if v1==1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2006 = aceiso_pdays_2006 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2006 = 365 if aceiso_pdays_2006>365
...
egen aceiso_push_2006 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2006 = 0 if aceiso_push_2006==.
replace aceiso_push_2006 = 0 if aceiso_push_2006<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2006p.dta", replace
*/
use "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2006p.dta", replace

keep bene_id aceiso_push_2006
tempfile push
save `push', replace
...
replace pushstock = pushstock + aceiso_push_2006 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2007 = sum(dayssply), by(idn)
egen aceiso_clms_2007 = count(dayssply), by(idn)
egen aceiso_minfilldt_2007 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2007 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2007 = aceiso_maxfilldt_2007 - aceiso_minfilldt_2007 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2007 = aceiso_pdays_2007 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2007 = 365 if aceiso_pdays_2007>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2007 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2007 = 0 if aceiso_push_2007==.
replace aceiso_push_2007 = 0 if aceiso_push_2007<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2007p.dta", replace

keep bene_id aceiso_push_2007
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceiso_push_2007 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2008 = sum(dayssply), by(idn)
egen aceiso_clms_2008 = count(dayssply), by(idn)
egen aceiso_minfilldt_2008 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2008 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2008 = aceiso_maxfilldt_2008 - aceiso_minfilldt_2008 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2008 = aceiso_pdays_2008 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2008 = 365 if aceiso_pdays_2008>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2008 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2008 = 0 if aceiso_push_2008==.
replace aceiso_push_2008 = 0 if aceiso_push_2008<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2008p.dta", replace

keep bene_id aceiso_push_2008
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceiso_push_2008 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2009 = sum(dayssply), by(idn)
egen aceiso_clms_2009 = count(dayssply), by(idn)
egen aceiso_minfilldt_2009 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2009 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2009 = aceiso_maxfilldt_2009 - aceiso_minfilldt_2009 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2009 = aceiso_pdays_2009 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2009 = 365 if aceiso_pdays_2009>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2009 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2009 = 0 if aceiso_push_2009==.
replace aceiso_push_2009 = 0 if aceiso_push_2009<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2009p.dta", replace

keep bene_id aceiso_push_2009
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceiso_push_2009 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2010 = sum(dayssply), by(idn)
egen aceiso_clms_2010 = count(dayssply), by(idn)
egen aceiso_minfilldt_2010 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2010 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2010 = aceiso_maxfilldt_2010 - aceiso_minfilldt_2010 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2010 = aceiso_pdays_2010 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2010 = 365 if aceiso_pdays_2010>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2010 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2010 = 0 if aceiso_push_2010==.
replace aceiso_push_2010 = 0 if aceiso_push_2010<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2010p.dta", replace

keep bene_id aceiso_push_2010
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceiso_push_2010 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2011 = sum(dayssply), by(idn)
egen aceiso_clms_2011 = count(dayssply), by(idn)
egen aceiso_minfilldt_2011 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2011 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2011 = aceiso_maxfilldt_2011 - aceiso_minfilldt_2011 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2011 = aceiso_pdays_2011 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2011 = 365 if aceiso_pdays_2011>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2011 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2011 = 0 if aceiso_push_2011==.
replace aceiso_push_2011 = 0 if aceiso_push_2011<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2011p.dta", replace

keep bene_id aceiso_push_2011
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceiso_push_2011 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2012 = sum(dayssply), by(idn)
egen aceiso_clms_2012 = count(dayssply), by(idn)
egen aceiso_minfilldt_2012 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2012 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2012 = aceiso_maxfilldt_2012 - aceiso_minfilldt_2012 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2012 = aceiso_pdays_2012 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2012 = 365 if aceiso_pdays_2012>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2012 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2012 = 0 if aceiso_push_2012==.
replace aceiso_push_2012 = 0 if aceiso_push_2012<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2012p.dta", replace

keep bene_id aceiso_push_2012
tempfile push
save `push', replace
...
//add the pushstock from last year
replace pushstock = pushstock + aceiso_push_2012 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen aceiso_fdays_2013 = sum(dayssply), by(idn)
egen aceiso_clms_2013 = count(dayssply), by(idn)
egen aceiso_minfilldt_2013 = min(srvc_dt), by(idn)
egen aceiso_maxfilldt_2013 = max(srvc_dt), by(idn)
gen aceiso_fillperiod_2013 = aceiso_maxfilldt_2013 - aceiso_minfilldt_2013 + 1
...
*/
forvalues d = 1/365{
	replace aceiso_pdays_2013 = aceiso_pdays_2013 + 1 if aceiso_a`d'==1
}
drop aceiso_a*
replace aceiso_pdays_2013 = 365 if aceiso_pdays_2013>365
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen aceiso_push_2013 = rowmax(lastfill_push inyear_push) 
replace aceiso_push_2013 = 0 if aceiso_push_2013==.
replace aceiso_push_2013 = 0 if aceiso_push_2013<0

keep bene_id aceiso* 

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_2013p.dta", replace
...
gen yaceiso2008 = 1
merge 1:1 bene_id using `aceiso'
drop _m
save `aceiso', replace
...
foreach i in `years'{
	replace aceiso_firstyoo = `i' if yaceiso`i'==1
}
local years "2007 2008 2009 2010 2011 2012 2013"
foreach i in `years'{
	replace aceiso_lastyoo = `i' if yaceiso`i'==1
}
gen aceiso_yearcount = aceiso_lastyoo - aceiso_firstyoo + 1

//utilization variables 
forvalues i = 2007/2013{
	replace aceiso_fillperiod_`i' = 0 if aceiso_fillperiod_`i'==.
	replace aceiso_clms_`i' = 0 if aceiso_clms_`i'==.
	replace aceiso_fdays_`i' = 0 if aceiso_fdays_`i'==.
	replace aceiso_pdays_`i' = 0 if aceiso_pdays_`i'==.
}	
//minfilldt maxfilldt push

//total utilization
gen aceiso_clms = aceiso_clms_2007+aceiso_clms_2008+aceiso_clms_2009+aceiso_clms_2010+aceiso_clms_2011+aceiso_clms_2012+aceiso_clms_2013
gen aceiso_fdays = aceiso_fdays_2007+aceiso_fdays_2008+aceiso_fdays_2009+aceiso_fdays_2010+aceiso_fdays_2011+aceiso_fdays_2012+aceiso_fdays_2013
gen aceiso_pdays = aceiso_pdays_2007+aceiso_pdays_2008+aceiso_pdays_2009+aceiso_pdays_2010+aceiso_pdays_2011+aceiso_pdays_2012+aceiso_pdays_2013

//timing variables 
egen aceiso_minfilldt = rowmin(aceiso_minfilldt_2007 aceiso_minfilldt_2008 aceiso_minfilldt_2009 aceiso_minfilldt_2010 aceiso_minfilldt_2011 aceiso_minfilldt_2012 aceiso_minfilldt_2013)
egen aceiso_maxfilldt = rowmax(aceiso_maxfilldt_2007 aceiso_maxfilldt_2008 aceiso_maxfilldt_2009 aceiso_maxfilldt_2010 aceiso_maxfilldt_2011 aceiso_maxfilldt_2012 aceiso_maxfilldt_2013)

gen aceiso_fillperiod = aceiso_maxfilldt - aceiso_minfilldt + 1

gen aceiso_pdayspy = aceiso_pdays/aceiso_yearcount
gen aceiso_fdayspy = aceiso_fdays/aceiso_yearcount
gen aceiso_clmspy = aceiso_clms/aceiso_yearcount

sort bene_id
drop if bene_id==bene_id[_n-1]

compress
save "/disk/agedisk3/medicare.work/goldman-DUA25731/PROJECTS/AD-Statins/Data/poly/aceiso_0713p.dta", replace
